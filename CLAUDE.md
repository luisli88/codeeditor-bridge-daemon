...
- One Go package per responsibility under `internal/` (`ws`, `gitmanager`, `entitlements`, `devpod`, `bootstrap`, `debug`, `session`, `filetree`) — no duplication between packages (Principio IV).
...
- `cmd/bridged/main.go` already wires everything that has a real implementation: the `lsp`/`shell`/`run`/`git`/`debug`/`fs` channels, and the handshake/git-credentials/provisioning HTTP routes — verified serving real traffic end to end against self-hosted (no AWS: `gitmanager.LocalFileStore` instead of `SecretsManagerStore`, `entitlements.Gate` built with a `nil` store since `CheckQuota` never touches it for `HostKind == "self-hosted"`). Not registered: `claude` (pushed from `shell`'s `OutputWatcher`, no handler of its own needed), `entitlements` (only called internally via `Provisioner`, no caller sends a direct WS envelope). `internal/filetree` (the `fs` channel) is read-only — list/read, no write/rename/delete/watch — see its own package doc comment.
- Every subprocess this daemon spawns on behalf of a Workspace now runs inside that Workspace's own devpod container via `devpod ssh <workspaceID> --command ...`, never directly on the Bridge Daemon's own host: `internal/ws/lsp_proxy.go`/`internal/debug` (live, piped subprocesses via `devpodexec.StartPiped`), `internal/devpod` (Run, via `devpodexec.StartWithOutputPipes`), `internal/session` (the Terminal's `tmux` session, via a direct `devpod ssh ... --command` `exec.Command` wrapped in `pty.Start`), and `internal/bootstrap.Executor` (LSP/Claude Code install, via `devpod.SSHExecutor`). Tests fake the `devpod` binary on `PATH` (`tests/devpodexec_fake_test.go`'s `fakeDevpodSSHScript`) rather than requiring a real Workspace.
